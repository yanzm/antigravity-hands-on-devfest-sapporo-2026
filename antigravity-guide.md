...
> **Note** : 以前のバージョンにあった「Plan」トグルはなくなりました。最初の依頼では自動的に **Implementation Plan** が作られ、**Proceed** を押してから実装が始まります。
...
| コマンド | 機能 | ハンズオンでの扱い |
| --- | --- | --- |
| `/plan` | コードを調査して、レビュー可能な実装計画を作る（以前の Plan トグルの代わり） | 大きめの変更の前に |
...
ハンズオンで配布するルールと、各項目の根拠は [haifu/README.md](haifu/README.md) を参照してください。
...
> **Note** : 以前のバージョンには「ターミナル実行ポリシー」に **Off / Auto / Turbo** という 3 つのモードがあり、Turbo は「拒否リストにあるもの以外は確認なしで実行する」設定でした。2.0 の公式ドキュメントにはこの名前が出てこず、許可リスト / 拒否リストと対話的な承認、そして「Terminal Sandboxing (Preview)」という説明になっています。2.0 のどこに相当する設定があるかは当日前に確認します。いずれにせよ、**「全部許可」に相当する設定にはしない**でください。エージェントが失敗したときの「調査モード」を止められなくなります。
