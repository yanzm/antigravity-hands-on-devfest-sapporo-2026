...
- **MCP を使う** : エージェントが Firestore のデータを直接読み書きしたり、Cloud Run にそのままデプロイしたりします。zip を作ってアップロードし、Cloud Shell でコマンドを叩く、といった手作業はありません
...
デプロイには 2〜3 分かかります。裏では「必要な API の有効化 → ソースの zip アップロード → Cloud Build でコンテナをビルド → Cloud Run にデプロイ → 認証なしアクセスの許可」が順番に走っています。MCP がなければ、これを自分で zip を作って Cloud Shell にアップロードし、`gcloud run deploy` を打って、と手作業でやることになります。
...
大きめの変更を頼む前に、コードを書かせずに計画だけ見たいときに使います。
